...
int main(int argc, char* argv[])
{
  ...
  printf("Mask written\n");
  cntl->channelSevr = uint64_t(-1ULL);
  ...
  int fd_mc;
  {
    ...
    saddr_mc.sin_addr.s_addr = htonl(MyType::IP);
    saddr_mc.sin_port        = htons(MyType::Port);
  ...
  BldServer<MyType> bldServer(fd_mc, MyType::Src);
  ...
  do {
    ...
    do {
      ...
      //  Generate BLD out
      MyType contents(ev.channels, ev.valid);
      bldServer.publish( ev.pulseId, ev.timeStamp, contents );
