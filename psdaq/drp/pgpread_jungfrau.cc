...
int main(int argc, char* argv[])
{
    ...
    while (1) {
        ...
        for (int b=0; b < ret; b++) {
            ...
            // inspect the jungfrau udp frame
            if (transition_id == XtcData::TransitionId::L1Accept) {
                // get current time to measure latency
                struct timespec current_ts;
                timespec_get(&current_ts, TIME_UTC);
                XtcData::TimeStamp readTime(current_ts);
                // unbatch the data
                EvtBatcherIterator ebit = EvtBatcherIterator(reinterpret_cast<EvtBatcherHeader*>(dmaBuffers[index]), size);
                ...
                size_t udpIdx = 2;
                if (subframes.size() > udpIdx) {
                    ebit = EvtBatcherIterator(reinterpret_cast<EvtBatcherHeader*>(subframes[udpIdx].data()), subframes[udpIdx].num_elem());
                    ebsft = ebit.next();
                    std::vector<XtcData::Array<uint8_t>> subframesUdp;
                    subframesUdp.reserve(JungfrauData::PacketNum);

                    do {
                        subframesUdp.emplace_back(ebsft->data(), &ebsft->size(), 1);
                    } while((ebsft=ebit.next()));
                    ...
                    // now check that the jungfrau frame is valid
                    for (size_t packetIdx = 0; packetIdx < subframesUdp.size(); packetIdx++) {
                        JungfrauData::JungfrauPacket* packet = reinterpret_cast<JungfrauData::JungfrauPacket*>(subframesUdp[packetIdx].data());
                        if (packetIdx == 0) {
                            framenum = packet->header.framenum;
                            timestamp = packet->header.timestamp;
                            frameTime = XtcData::TimeStamp(timestamp);
                        } else {
                            if (valid_frame && packet->header.framenum != framenum) {
                                valid_frame = false;
                                printf("packet[%zu] unexpected framenum %lu [%lu]\n", packetIdx, packet->header.framenum, framenum);
                            }
                            if (valid_ts && packet->header.timestamp != timestamp) {
                                valid_ts = false;
                                printf("packet[%zu] unexpected timestamp %lu [%lu]\n", packetIdx, packet->header.timestamp, timestamp);
                            }
                        }
                        if (valid_packets && packet->header.packetnum != subframesUdp.size() - packetIdx - 1) {
                            valid_packets = false;
                            printf("packet[%zu] unexpected packetnum %u\n", packetIdx, packet->header.packetnum);
                        }
                    }
                    double unix_ts = frameTime.asDouble();
                    double delta = readTime.asDouble() - unix_ts;
                    if (valid_frame && valid_ts && valid_num_packet && valid_packets) {
                        printf("Jungfrau frame %lu ts %f (latency %f) contains %zu packets and is valid\n",
                               framenum, unix_ts, delta, subframesUdp.size());
                    } else {
                        const char* error = nullptr;
                        if (!valid_frame) {
                            error = "framenum mismatch";
                        } else if (!valid_ts) {
                            error = "timestamp mismatch";
                        } else if (!valid_num_packet) {
                            error = "wrong number of packets";
                        } else if (!valid_packets) {
                            error = "wrong packet order";
                        } else {
                            error = "unknown";
                        }
                        printf("Jungfrau frame %lu ts %f (latency %f) contains %zu packets and is invalid: %s\n",
                               framenum, unix_ts, delta, subframesUdp.size(), error);
                    }
                }
            }
