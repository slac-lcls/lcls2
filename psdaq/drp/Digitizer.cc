    ...
    for (int i=0; i<PGP_MAX_LANES; i++) {
        if (event->mask & (1 << i)) {
            ...
            do {
                ...
                if (stream.overflow()) { // recoverable
                    logging::debug("Overflow error");
                    dgram.xtc.damage.increase(Damage::UserDefined);
                    break;
                }
                if (stream.unlocked()) {  // unrecoverable
                    logging::error("JESD unlocked");
                    dgram.xtc.damage.increase(Damage::UserDefined);
                    throw("JESD unlocked.  Need to assert jesdinit or restart hsdioc");
                }
