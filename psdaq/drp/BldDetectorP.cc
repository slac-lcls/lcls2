...
void Pgp::worker(std::shared_ptr<Pds::MetricExporter> exporter)
{
    ...
    time_t tt0;
    {
        ...
        tm_s.tm_yday = 0;
        tm_s.tm_isdst = 0;
        // time_t tt = mktime(&tm_s);
        // tt0 = mktime(gmtime(&tt));
        time_t tt = timegm(&tm_s);
        tt0 = mktime(localtime(&tt));
    }
    */
    ...
    while (true) {
        ...
        /*
        if (dgram && ((skipPoll&(1<<0)) || pfd[0].events == POLLIN)) 
            logging::warning("dgram overwrite  skipPoll 0x%x",skipPoll);
        */
        ...
        if (rval < 0) {  // error
            ...
            /*
            if (lRunning)
                logging::debug("poll rval[%d]  pfd[0].revents %u  pfd[1].revents %u  dgram %p  doPoll %c  skipPoll 0x%x",
                               rval, pfd[0].revents,pfd[1].revents,dgram,lDoPoll?'t':'f',skipPoll);
            */
            lDoPoll = false;
