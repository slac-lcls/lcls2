...
static inline unsigned getf(const uint32_t& i, unsigned n, unsigned sh)
{
  unsigned v = i;
  return (v>>sh)&((1<<n)-1);
}

static inline unsigned setf(uint32_t& o, unsigned v, unsigned n, unsigned sh)
{
                      ...
                      bool        lsparse)
{
  ...
  if ((control & (1<<8))==0) {
    ...
    setf(_es_control, 1, 1, 8);  // eyescan_en
