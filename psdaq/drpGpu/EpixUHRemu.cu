  ...
  Drp::Gpu::Detector(&para, &pool)
{
  ...
  chkError(cudaMalloc(&m_pedArr_d,  m_dets.size() * sizeof(*m_pedArr_d)));
  chkError(cudaMalloc(&m_gainArr_d, m_dets.size() * sizeof(*m_gainArr_d)));

  // Allocate space for the calibration constants for each panel
  m_pedsVec_d.resize(m_dets.size());
  m_gainsVec_d.resize(m_dets.size());
