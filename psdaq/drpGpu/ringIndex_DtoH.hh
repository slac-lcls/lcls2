...
//    ////printf("*** rb::prepare 2, next %d\n", next);
//    //m_cursor->store(next, cuda::memory_order_release);
//    ////printf("*** rb::prepare 3, cursor %d, ret %d\n", m_cursor->load(), idx);
...
//  }
  ...
  {
    printf("*** rb::produce 1, idx %d, head %d\n", idx, m_head->load());
    auto next = (idx+1)&(m_capacity-1);
    printf("*** rb::produce 2, nxt %d\n", next);
...
//  __device__ unsigned head() const
//  {
//    return m_head.load(cuda::memory_order_acquire);
//  }
//
//  __device__ unsigned tail() const
//  {
