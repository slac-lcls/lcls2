...
def main():
    ...
    aargs = [
        (
            "--gain-modes",
            {
                ...
                "help": (
                    "Gain modes to use (Default: "
                    "['FHG', 'FMG', 'FLG1', 'FLG2', 'AHLG1', 'AHLG2', 'AMLG1', 'AMLG2'])"
                ),
            },
        ),
    ]
    ...
    def steps():
        ...
        for i in range(args.nprocs):
            ...
            d[f"{args.detname}_{i}:user.Gain.UsePixelMap"] = 0
            d[f"{args.detname}_{i}:user.Gain.PixelBitMapSel"] = 0
            d[f"{args.detname}_{i}:user.FebFpga.App.VINJ_DAC.enable"] = 0
            d[f"{args.detname}_{i}:user.FebFpga.App.VINJ_DAC.dacEn"] = 0
            d[f"{args.detname}_{i}:user.FebFpga.App.VINJ_DAC.rampEn"] = 0
            d[f"{args.detname}_{i}:user.FebFpga.App.VINJ_DAC.dacStartValue"] = 0
            d[f"{args.detname}_{i}:user.FebFpga.App.VINJ_DAC.dacStopValue"] = 0
            d[f"{args.detname}_{i}:user.FebFpga.App.VINJ_DAC.dacStepValue"] = 0
