#include "psdaq/hsd/src/Fmc134Cpld.hh"
...
static void _adc_cal_dump_reg(Fmc134Cpld& cpld,
                              json& j,
                              unsigned adc,
                              const char* title,
                              unsigned nb,
                              unsigned reg) 
{
    uint32_t dword0, word0;
    int32_t rc = UNITAPI_OK;
    Fmc134Cpld::DevSel adcsel = adc==0 ? Fmc134Cpld::ADC0_SELECT : Fmc134Cpld::ADC1_SELECT;
    if (nb>8) {
        word0 = cpld.readRegister(adcsel,reg+0);
        if (rc!=UNITAPI_OK) {
            logging::error("Error reading ADC0_SELECT reg 0x%x\n", reg);
            return;
        }
        dword0 = cpld.readRegister(adcsel, reg+1);
        word0 = (word0&0xff) | ((dword0&0xff)<<8);
    }
    else {
        word0 = cpld.readRegister(adcsel,reg);
    }
    word0 &= ((1<<nb)-1);
    j[title] = word0;
}

std::string Fmc134Cpld::adc_cal_dump(unsigned adc) {
    json j;
    _adc_cal_dump_reg(*this,j,adc,"OADJ_A_VINA",12,0x8A);
    _adc_cal_dump_reg(*this,j,adc,"OADJ_A_VINB",12,0x8C);
    _adc_cal_dump_reg(*this,j,adc,"OADJ_B_VINA",12,0x92);
    _adc_cal_dump_reg(*this,j,adc,"OADJ_B_VINB",12,0x94);
    _adc_cal_dump_reg(*this,j,adc,"GAIN_TRIM_A", 8,0x7A);
    _adc_cal_dump_reg(*this,j,adc,"GAIN_TRIM_B", 8,0x7B);
    _adc_cal_dump_reg(*this,j,adc,"B0_TIME_0"  , 8,0x102);
    _adc_cal_dump_reg(*this,j,adc,"B0_TIME_90" , 8,0x103);
    _adc_cal_dump_reg(*this,j,adc,"B1_TIME_0"  , 8,0x112);
    _adc_cal_dump_reg(*this,j,adc,"B1_TIME_90" , 8,0x113);
    _adc_cal_dump_reg(*this,j,adc,"B4_TIME_0"  , 8,0x142);
    _adc_cal_dump_reg(*this,j,adc,"B4_TIME_90" , 8,0x143);
    _adc_cal_dump_reg(*this,j,adc,"B5_TIME_0"  , 8,0x152);
    _adc_cal_dump_reg(*this,j,adc,"B5_TIME_90" , 8,0x153);
    _adc_cal_dump_reg(*this,j,adc,"TADJ_A_FG90", 8,0x80);
    _adc_cal_dump_reg(*this,j,adc,"TADJ_B_FG0" , 8,0x81); 
    json cnt;
    cnt["expert"] = j;
    return cnt.dump();
}

void Fmc134Cpld::adc_cal_load(unsigned adc, const std::string& str) {

#define _check(ctn,field) {                                             \
        if (!ctn.contains(field)) {                                     \
            logging::error("Attempted to write calib with no %s field",field); \
            return;                                                     \
        }                                                               \
    }

    if (!str.empty()) {
        json calib = json::parse(str.c_str());
        
        _check(calib,"expert");
        json expert = calib["expert"];
        
        _check(expert,"OADJ_A_VINA");
        _check(expert,"OADJ_B_VINA");
                
#define calib_write(addr,title) {                               \
            uint16_t v = expert[title].get<uint16_t>();         \
            logging::info("Writing %s = %x",title,v);           \
            spi_write(0, adc_select, addr+0, v&0xff);           \
            spi_write(0, adc_select, addr+1, v>>8); }

        DevSel adc_select = adc==0 ? ADC0_SELECT : ADC1_SELECT;
        calib_write(0x8A,"OADJ_A_VINA");
        calib_write(0x92,"OADJ_B_VINA");
    }
    else {
        logging::info("No calib to write");
    }
}
