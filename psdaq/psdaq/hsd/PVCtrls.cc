...
#define NPV1(name)  _pv.push_back( new PvServer(STOU(pvbase+#name).c_str()) )
    ...
    enum PvIndex { Enable, 
                   ...
                   FullEvt, FullSize,
                   TestPattern, SyncELo, SyncEHi, SyncOLo, SyncOHi, TrigShift, 
      ...
      for(unsigned i=0; i<4; i++) {
        ...
        if ((1<<i)&channelMask) {
          ...
          if (TON(_pv[Raw_PS]->data(),i)) {
            streamMask |= (1<<0);
            fex._base[0].setGate(TON(_pv[Raw_Start]->data(),i),
                                 TON(_pv[Raw_Gate ]->data(),i));
            fex._base[0].setFull(fullSize,fullEvt);
            fex._base[0]._prescale=TON(_pv[Raw_PS]->data(),i)-1;
          }
          if (TON(_pv[Fex_PS]->data(),i)) {
            streamMask |= (1<<1);
            fex._base[1].setGate(TON(_pv[Fex_Start]->data(),i),
                                 TON(_pv[Fex_Gate ]->data(),i));
          ...
          if (TON(_pv[Nat_PS]->data(),i)) {
            streamMask |= (1<<2);
            fex._base[2].setGate(TON(_pv[Nat_Start]->data(),i),
                                 TON(_pv[Nat_Gate ]->data(),i));
