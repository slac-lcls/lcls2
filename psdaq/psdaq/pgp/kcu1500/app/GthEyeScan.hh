...
namespace Kcu {
    ...
    public:
        ...
        Reg      _es_control;  // 0x0f0: [15:10] control, [9] errdet_en, [8] eye_scan_en, [4:0] prescale
        uint32_t _reserved_3f[0x3f-0x3d];
        Reg      _es_qualifier[5];   // 0x0fc
        Reg      _es_qual_mask[5];   // 0x110
        Reg      _es_sdata_mask[5];  // 0x124
        uint32_t _reserved_rf[0x4f-0x4e];
        Reg      _es_horz_offset;    // 0x13c: [15:4]
        uint32_t _reserved_97[0x97-0x50];
        Reg      _rx_eyescan_vs; // 0x25c: [10] ut_sign, [9] neg_dir, [8:2] code (vert_offset), [1:0] range
        uint32_t _reserved_151[0x151-0x98];
        Reg      _es_error_count;    // 0x544
        Reg      _es_sample_count;   // 0x548
        Reg      _es_control_status; // 0x54c
