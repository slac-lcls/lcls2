...
//
//  Modified interaction for I2C
//
typedef volatile uint32_t vuint32_t;
static uint32_t _proxy_base;
static uint32_t _proxy_csr;

static void _initialize_proxy(unsigned base, unsigned csr)
{
  _proxy_base = base;
  _proxy_csr  = csr;
}

static inline uint32_t get_reg32_proxy(unsigned addr) {
  int reg = addr - _proxy_base;
  set_reg32(_proxy_csr+(2<<2),reg);
  set_reg32(_proxy_csr+(0<<2),1);
  ...
  do { 
    ...
    if ((++tmo&tmo_mask) ==  tmo_mask) {
      ...
      printf("RegProxy tmo (%x) read from 0x%x\n", 
             tmo, reg);
    }
  } while ( (get_reg32(_proxy_csr+(1<<2))&1)==0 );

  return get_reg32(_proxy_csr+(3<<2));
}

static inline void set_reg32_proxy(unsigned addr, uint32_t value) {
  int reg = addr - _proxy_base;
  //  launch transaction
  set_reg32(_proxy_csr+(3<<2),value);
  set_reg32(_proxy_csr+(2<<2),reg);
  set_reg32(_proxy_csr+(0<<2),0);
  ...
  do { 
    ...
    if ((++tmo&tmo_mask) ==  tmo_mask) {
      tmo_mask = (tmo_mask<<1) | 1;
      printf("RegProxy tmo (%x) writing 0x%x to 0x%x\n", 
             tmo, value, reg);
    }
  } while ( (get_reg32(_proxy_csr+(1<<2))&1)==0 );
}
...
{
  unsigned i2c_mux = get_reg32_proxy(0x00e00000);
  printf("i2c_mux : 0x%x\n", i2c_mux);
  i2c_mux = (1<<2);
  set_reg32_proxy(0x00e00000,i2c_mux);
  printf("i2c_mux : 0x%x\n", i2c_mux);
}
...
//  Reset to factory defaults
  ...
  unsigned v = get_reg32_proxy(SI570(135));
  v |= 1;
  set_reg32_proxy(SI570(135), v);
  do { usleep(100); } while (get_reg32_proxy(SI570(135))&1);
}
...
static double read_si570()
{
  ...
  unsigned v = get_reg32_proxy(SI570(7));
  unsigned hs_div = hsd_divn[(v>>5)&7];
  unsigned n1 = (v&0x1f)<<2;
  v = get_reg32_proxy(SI570(8));
  n1 |= (v>>6)&3;
  uint64_t rfreq = v&0x3f;
  rfreq <<= 32;
  rfreq |= ((get_reg32_proxy(SI570( 9))&0xff)<<24) |
    ((get_reg32_proxy(SI570(10))&0xff)<<16) |
    ((get_reg32_proxy(SI570(11))&0xff)<< 8) |
    ((get_reg32_proxy(SI570(12))&0xff)<< 0);
...
static void set_si570(double f)
{
  ...
  //  Freeze DCO
  unsigned v = get_reg32_proxy(SI570(137));
  v |= (1<<4);
  set_reg32_proxy(SI570(137),v);

  unsigned hs_div = 3; // =7
  unsigned n1     = 3; // =4
  uint64_t rfreq  = uint64_t(5200. / f * double(1<<28));

  set_reg32_proxy(SI570( 7),((hs_div&7)<<5) | ((n1>>2)&0x1f));
  set_reg32_proxy(SI570( 8),((n1&3)<<6) | ((rfreq>>32)&0x3f));
  set_reg32_proxy(SI570( 9),(rfreq>>24)&0xff);
  set_reg32_proxy(SI570(10),(rfreq>>16)&0xff);
  set_reg32_proxy(SI570(11),(rfreq>> 8)&0xff);
  set_reg32_proxy(SI570(12),(rfreq>> 0)&0xff);

  printf("Wrote: hs_div %x  n1 %x  rfreq %lx  f %f MHz\n",
         hs_div, n1, rfreq, f);

  //  Unfreeze DCO
  v = get_reg32_proxy(SI570(137));
  v &= ~(1<<4);
  set_reg32_proxy(SI570(137),v);

  v = get_reg32_proxy(SI570(135));
  v |= (1<<6);
  set_reg32_proxy(SI570(135),v);
}
...
static void dumpAxiStreamMon(unsigned a)
{
  unsigned v = get_reg32(a+0x10); // frameRate
  unsigned w = get_reg32(a+0x14); // validCount
  unsigned x = get_reg32(a+0x18); // nReadyCount
  uint64_t z = get_reg32(a+0x20);
  z <<= 32;
  z |=         get_reg32(a+0x1c);
  printf("%10u %10u %10u %10u %16llu %16.4f\n",
         v,w,x,w-x,z,double(z)/double(w-x));
}
...
int main(int argc, char* argv[])
{
    ...
    int c;
    while((c = getopt(argc, argv, "cd:sStTmMFDC:")) != EOF) {
      switch(c) {
    ...
    if (core_pcie) {
      ...
      if (setup_clk) {
        ...
        _initialize_proxy(0x00e00000, 0x00e00000);
    ...
    /*
    printf("%10.10s %10.10s %10.10s %10.10s %16.16s %16.16s | ",
           "frameRate","tValid","-tReady","+tReady",
           "byteRate","byte/tReady");
    */
    ...
    while (monDma) {
      usleep(1000000);
      dumpAxiStreamMon(0x00060000); // into PCIe DMA
      dumpAxiStreamMon(0x00070000); // PCIe axi wvalid  (after xbar)
      dumpAxiStreamMon(0x00070030); // PCIe axi awvalid
      dumpAxiStreamMon(0x00F00000); // into local RAM DMA
      printf("\n");
    }
    return 0;
}
