...
class SbatchManager:
    ...
    def get_jobstep_cmd(
        ...
        if not as_step:
            jobstep_cmd = f"srun -n1 -c{n_cores} --cpu-bind=cores --unbuffered --job-name={job_name} {het_group_opt}{output_opt}{env_opt}bash -c '{cmd}'"
        else:
            jobstep_cmd = (
                f"srun -n1 --exclusive --cpus-per-task={n_cores} --cpu-bind=cores --unbuffered --job-name={job_name} {het_group_opt}{output_opt}{env_opt}bash -c '{cmd}'"
                + "&\n"
            )
        return jobstep_cmd
