...
- `daqmgr` writes process logs under `<log_root>/YYYY/MM`.
- `--output` (or `-o`) sets `<log_root>`.
- If `--output` is not provided, default `<log_root>` is `$HOME/daq/logs`
  for every user. `daqmgr` creates this directory and the `YYYY/MM` subdirectories
  when it starts.
