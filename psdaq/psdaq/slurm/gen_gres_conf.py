...
then paste, then reconfigure.  slurm.conf also needs

    GresTypes=gpu,datadev

or the datadev gres is unknown and requests for it fail, and the node's own line needs
a matching Gres=, which this prints for you.
...
with '-d /dev/datadev_84' requests '--gres=gpu:dd84:1,datadev:dd84:1' and the two
...
BEGIN = "# >>> gen_gres_conf.py: {node} >>>"
END   = "# <<< gen_gres_conf.py: {node} <<<"


def emit(pairs, node, counts):
    """This node's records, with the comments needed to read them at 3 a.m."""
    import datetime, getpass
    stamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    out = [BEGIN.format(node=node),
           f"# Generated by gen_gres_conf.py on {node} at {stamp} "
           f"by {getpass.getuser()}",
           f"# {counts}",
           "#",
           "# Every line is scoped with NodeName= because the controller distributes",
           "# one global gres.conf to all nodes.",
           "#",
           "# Type names are the datadev's PCI bus number, so a process launched with",
           "# '-d /dev/datadev_XX' requests '--gres=gpu:ddXX:1,datadev:ddXX:1'.  The",
           "# name is bound to the *card*; which GPU serves it is decided solely by",
           "# the File= on its Name=gpu line.  So to move a card to a spare GPU --",
           "# because one died, say -- change that one File= and run",
           "# 'scontrol reconfigure'.  Nothing else has to change: not slurm.conf's",
           "# Gres=, not the DAQ configuration.  (Cores= will then name the old GPU's",
           "# cores, which costs affinity and nothing else.)"]
    out.append("")
    ...
    for p in sorted(pairs, key=lambda p: p["datadev"]["pci"]):
        ...
        out += [f"# {card['name']} {card['pci']} <-> GPU {gpu['pci']} "
                f"({gpu['dev']}), {where}, NUMA {numa}",
                f"NodeName={node} Name=gpu     Type={typ} File={gpu['dev']}{suffix}",
                f"NodeName={node} Name=datadev Type={typ} File={card['dev']}{suffix}",
                ""]
        for name in ("gpu", "datadev"):
            gres_counts[f"{name}:{typ}"] = 1

    spec = ",".join(f"{k}:1" for k in sorted(gres_counts))
    out += ["# slurm.conf needs this node's line to declare what is here, e.g.",
            f"#   NodeName={node} ... Gres={spec}",
            END.format(node=node)]
    return "\n".join(out) + "\n"
...
def main():
    ...
    if args.check:
        try:
            ...
            return 1
        want = _records(emit(pairs, node, counts), node)
        if have == want:
    ...
    print(f"info: paste this into psslurmctld001:/etc/slurm/gres.conf, replacing any "
          f"existing {node} lines, then 'sudo scontrol reconfigure' there",
          file=sys.stderr)
    return 0
