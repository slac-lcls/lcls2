...
laser_delay = L_dark + M_pre + pre_delay + 1 # 15ms to open Uniblitz
laser_width = 0.6 # 0.2 10Hz shots = 20ms

# Assumes a 10Hz event rate
#event_to_ns_factor = 1E+8 # 100ms
event_to_ns_factor = 0.033333 # actually seconds
...
ebeam_delay = ebeam_delay * event_to_ns_factor
ebeam_width = ebeam_width * event_to_ns_factor
laser_delay = (laser_delay * event_to_ns_factor) - 0.015
laser_width = laser_width * event_to_ns_factor
#print("----")
#print(ebeam_delay, ebeam_width, laser_delay, laser_width)
...
epics.caput("UED:DDG:02:bDelayAO", ebeam_width)
epics.caput("UED:DDG:02:aDelayAO", ebeam_delay)
epics.caput("UED:DDG:01:bDelayAO", laser_width)
epics.caput("UED:DDG:01:aDelayAO", laser_delay)

pvSeqMask=Pv("DAQ:UED:XPM:0:PART:0:SeqMask")
# causes two engines to start at the same time (3 is 0b11 in binary)
pvSeqMask.put(3)

f=open("/path/to/ued/daq/release/psdaq/psdaq/seq/ued_engine_30hz.py")
enginelines=f.read()
f.close()
