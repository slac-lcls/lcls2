...
#define COUNTRESET_OFFSET   0x00
#define LOOPBACK_OFFSET     0x08
#define CELLERRCNT_OFFSET   0x14
#define LINKDOWNCNT_OFFSET  0x18
#define LINKERRCNT_OFFSET   0x1c

#define NPORTS              7

#define POLL_USEC           1000000 // usleep() argument
#define PRINT_SEC           120     // print period
...
public:
  uint32_t getPgp(unsigned i, unsigned addr) const {
      /* i range: 0-6 */
  ...
  // dumpTest
  void dumpTest(int addr, int offset, int mask) const {
    uint32_t reg;
    printf("[%s(addr=0x%02x, offset=%d, mask=0x%02x)]\n", __FUNCTION__, addr, offset, mask);
...
void errCountInit(unsigned *samples, unsigned *previous, unsigned *sum) {
  if (_verbose) {
    printf("[%s]\n", __FUNCTION__);
  }

  for (unsigned i=0; i<NPORTS; i++) {
    previous[i] = samples[i];     // update previous
    sum[i] = 0;                   // update sum
  }
}

void errCountReport(unsigned linkUp, unsigned *sum, unsigned duration) {
  printf("[%s]\n", __FUNCTION__);
  printf("linkUp               : %08x\n", linkUp);
  printf("duration             : %u sec\n", duration);
  printf("linkErrCnt sum       :");
  for (unsigned i=0; i<NPORTS; i++) {
    printf(" %08u", sum[i]);
  }
  printf("\nlinkErrCnt err/second:");
  for (unsigned i=0; i<NPORTS; i++) {
    printf(" %8.3f", 1.0 * sum[i] / duration);
  }
  printf("\nlinkErrCnt err/minute:");
  for (unsigned i=0; i<NPORTS; i++) {
    printf(" %8.3f", 60.0 * sum[i] / duration);
  }
  printf("\n\n");
}

void errCountUpdate(unsigned *samples, unsigned *previous, unsigned *sum) {

  if (_verbose) {
    printf("[%s]\n", __FUNCTION__);
    printf("samples   : ");
    for (unsigned i=0; i<NPORTS; i++) {
      printf("       %02u", samples[i]);
    }
    printf("\nprevious  : ");
    for (unsigned i=0; i<NPORTS; i++) {
      printf("       %02u", previous[i]);
    }
  }

  for (unsigned i=0; i<NPORTS; i++) {
    if (samples[i] > previous[i]) {
      sum[i] += (samples[i] - previous[i]);       // update sum
      previous[i] = samples[i];                   // update previous
    } else if (samples[i] < previous[i]) {
      sum[i] += (16 + samples[i] - previous[i]);  // update sum
      previous[i] = samples[i];                   // update previous
    }
  }
  if (_verbose) {
...
int main(int argc, char** argv) {
  ...
  int c;
  unsigned samples[NPORTS], previous[NPORTS], sum[NPORTS];
  time_t startTime, pollTime, printTime;
  bool lDumpTest = false;
  ...
  const char* ifile=0;
  char* endptr;
  while ( (c=getopt( argc, argv, "a:t:ru:f:L:kvHhT")) != EOF ) {
    switch(c) {
    case 'a': ip = optarg; break;
    case 't': _partn = strtoul(optarg, NULL, 0); break;
    case 'T': lDumpTest = true; break;
  ...
  for (unsigned i=0; i<NPORTS; i++) {
    samples[i] = dti->getPgp(i, LINKERRCNT_OFFSET) & 0xf;
  }
  errCountInit(samples, previous, sum);

  while(1) {
    usleep(POLL_USEC);
    pollTime = time(NULL);
    if (pollTime == printTime) {
      printFlag = false;
    } else {
      if ((pollTime - startTime) && ((pollTime - startTime) % PRINT_SEC == 0)) {
        printFlag = true;
        printTime = pollTime;
      } else {
        printFlag = false;
    ...
    if (printFlag) {
      errCountReport(dti->linkUp(), sum, PRINT_SEC);
    }
    ...
    errCountUpdate(samples, previous, sum);
  }
