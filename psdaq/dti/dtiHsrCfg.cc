...
int main(int argc, char** argv)
{
  ...
  Pds::Cphw::AmcTiming* t = &dti->_timing;
  printf("buildStamp %s\n",t->version.buildStamp().c_str());
  if (!strcasestr(t->version.buildStamp().c_str(), "DTI"))
    printf("*** WARNING *** Module does not appear to be a DTI!\n");

  //  dti->init();

  HsRepeater*  hsr = dti->_hsRepeater;
  ...
  if (scan)
  {
    ...
    for (unsigned i = 0; links; ++i)
    {
      ...
      if (links & (1<<i))
      {
        if (i < nUsLinks) {
          dti->usLink(i);
          printf("DTI US link %d: ", i); // No \n on purpose
        }
        else {
          dti->dsLink(nDsLinks - 1 - (i - nUsLinks));
          printf("DTI DS link %d: ", nDsLinks - 1 - (i - nUsLinks)); // No \n on purpose
        }

        Cphw::Reg* rxErrs = i < nUsLinks ? &dti->_usStatus._rxErrs : &dti->_dsStatus._rxErrs;
        unsigned   idx    = hsr[hsrMap[i]].scanLink(chnMap[i], chA, chB, dtiMeasFn, rxErrs);
