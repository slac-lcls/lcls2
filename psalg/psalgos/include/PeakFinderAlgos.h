...
{
  if(m_pbits & LOG::DEBUG) std::cout << "in _makeMapOfConnectedPixelsForLocalMaximums\n";
  ...
  for(int r=0; r<(int)m_rows; r++)
    for(int c=0; c<(int)m_cols; c++) {
        ...
        ++m_numreg;
        //std::cout << irc << " " << m_numreg << std::endl;
        ...
        //std::cout << "len: " << vv_peak_pixinds.size() << " " << m_npksmax << std::endl;
                     ...
                     )
{
  ...
  for(vector<TwoIndexes>::const_iterator ij  = v_indexes.begin(); ij != v_indexes.end(); ij++) {
    ...
    sum1 += amp;
    sum2 += amp*amp;
    //std::cout << sum1 << "," << sum2 << "," << sum0 << std::endl;
  }
  ...
  if(sum0) {
    ...
    //std::cout << "sum: " << sum1 << "," << sum2 << "," << sum0 << std::endl;
  }
