...
AreaDetectorCspad::AreaDetectorCspad(const std::string& detname, XtcData::ConfigIter& configo)
  : AreaDetector(detname, configo) {
  MSG(DEBUG, "In c-tor AreaDetectorCspad(detname, configo) for " << detname);
  process_config();
}

AreaDetectorCspad::AreaDetectorCspad(const std::string& detname)
  : AreaDetector(detname) {
  MSG(DEBUG, "In c-tor AreaDetectorCspad(detname) for " << detname);
}

AreaDetectorCspad::~AreaDetectorCspad() {
  MSG(DEBUG, "In d-tor AreaDetectorCspad for " << detname());
}

void AreaDetectorCspad::_class_msg(const std::string& msg) {
  MSG(INFO, "In AreaDetectorCspad::"<< msg);
}

  /*
const shape_t* AreaDetectorCspad::shape(const event_t&) {
  _class_msg("shape(...)");
  return &AreaDetector::_shape[0];
  //return &_shape[0];
}

const size_t AreaDetectorCspad::ndim(const event_t&) {
  _class_msg("ndim(...)");
  return 0;
}

const size_t AreaDetectorCspad::size(const event_t&) {
  _class_msg("size(...)");
  return 123;
}

const shape_t* AreaDetectorCspad::shape() {
  _class_msg("shape(...)");
  return &AreaDetector::_shape[0];
}

const size_t AreaDetectorCspad::size() {
  _class_msg("size(...)");
  return 123;
}
  */
...
const void AreaDetectorCspad::print_config() {
...
void AreaDetectorCspad::_panel_id(std::ostream& os, const int ind) {
...
void AreaDetectorCspad::detid(std::ostream& os, const int ind) {
...
NDArray<raw_cspad_t>& AreaDetectorCspad::raw(XtcData::DescData& ddata) {
    if(_ind_data < 0) _set_index_data(ddata, "frame");
    raw_cspad_t* pdata = ddata.get_array<raw_cspad_t>(_ind_data).data();
...
std::string AreaDetectorCspad::detid(const int& ind) {
  std::stringstream ss;
  detid(ss, ind);
  return ss.str();
}
