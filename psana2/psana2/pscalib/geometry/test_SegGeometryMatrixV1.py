...
if __name__ == "__main__":
  ...
  from psana2.pscalib.geometry.SegGeometryMatrixV1 import *
  import sys
...
import psana2.pyalgos.generic.Graphics as gg
  ...
  def test_xyz_maps():
    ...
    for i,arr2d in enumerate( w.get_seg_xy_maps_pix() ):
        ...
        gg.plotImageLarge(arr2d, amp_range=amp_range, figsize=(10,9), title=titles[i])
        gg.move(200*i,100*i)
  ...
  def test_img_easy():
    ...
    gg.plotImageLarge(img, amp_range=(xmin+2*ymin, xmax+2*ymax), figsize=(11,10))
    gg.show()
  ...
  def test_mask(width=0, edge_rows=5, edge_cols=5):
    ...
    mask = 1 + o.pixel_mask_array(width=width, edge_rows=edge_rows, edge_cols=edge_cols)
