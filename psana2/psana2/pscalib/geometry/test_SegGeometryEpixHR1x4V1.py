...
if __name__ == "__main__":
  ...
  from psana2.pscalib.geometry.SegGeometryEpixHR1x4V1 import *
...
import psana2.pyalgos.generic.Graphics as gg # For test purpose in main only
  ...
  def test_xyz_maps():
    ...
    for i,arr2d in enumerate( w.get_seg_xy_maps_pix() ):
        ...
        gg.plotImageLarge(arr2d, amp_range=amp_range, figsize=(20,4), title=titles[i], window=WINDOW)
        gg.move(200*i,100*i)
  ...
  def test_1x4_img_easy():
    ...
    gg.plotImageLarge(img, amp_range=(0, 750), figsize=(20,4), window=WINDOW)
    gg.show()
  ...
  def usage(tname='0'):
    ...
    if tname in ('0','4'): s+='\n 4 - test_1x4_img_easy()'
    if tname in ('0','5'): s+='\n 5 - test_pix_sizes()'
    if tname in ('0','6'): s+='\n 6 - test_1x4_mask(width=4, wcenter=6)'
    if tname in ('0','7'): s+='\n 7 - test_1x4_mask(width=0, wcenter=0, edge_rows=3, edge_cols=6, center_rows=2, center_cols=4)'
    return s
...
if __name__ == "__main__":
    ...
    elif tname in ('3',): test_1x4_img()
    elif tname in ('4',): test_1x4_img_easy()
    elif tname in ('5',): test_pix_sizes()
    elif tname in ('6',): test_1x4_mask(width=4, wcenter=6)
    elif tname in ('7',): test_1x4_mask(width=0, wcenter=0, edge_rows=10, edge_cols=6, center_rows=2, center_cols=4)
