...
import psana2.xtcav.ClusteringUtils as cu
try:
    from collections.abc import Mapping
except ImportError:
    from collections import Mapping

from psana2.pscalib.calib.XtcavUtils import xtcav_calib_object_from_dict, info_xtcav_object
...
def findROI(masks, ROI, expandfactor=1):
    ...
    ind1Y = max(0, np.round(centery - widthy/2).astype(np.int32))
    ind2Y = min(np.round(centery + widthy/2).astype(np.int32), rows.size)
    ind1X = max(0, np.round(centerx - widthx/2).astype(np.int32))
    ind2X = min(np.round(centerx + widthx/2).astype(np.int32), cols.size)
