import numpy as np
from psana2.momentum.CalcPzArr import CalcPzArr
class IonMomentumFlat():
    ...
    l_mm=None,d_mm=None,ls_mm=None,U_V=None,Es_VPmm=None):
        ...
        self.d = d_mm
        if ls_mm is None:
            self.ls = np.array([self.l,self.d],dtype=np.float32)
        else:
            self.ls = np.array(ls_mm,dtype=np.float32)
        if Es_VPmm is None:
            self.Es = np.array([self.U/self.l,0],dtype=np.float32)
        else:
            self.Es = np.array(Es_VPmm,dtype=np.float32)
    ...
    def CalcPzMultiAcc(self,m_amu,q_au,t_ns):
        if isinstance(t_ns,(list, tuple, np.ndarray)):
            ...
            if not isinstance(q_au,(list, tuple, np.ndarray)):
                q_au = np.ones((len(t_ns),),dtype=np.float32)*q_au
