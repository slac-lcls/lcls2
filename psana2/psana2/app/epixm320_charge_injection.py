...
from psana2.detector.dir_root import DIR_REPO_EPIXM320 # DIR_REPO_EPIX10KA
...
def do_main():
    ...
    from psana2.detector.UtilsEpixm320ChargeInjection import charge_injection
    t0_sec = time()
...
USAGE = 'TEST EXAMPLE'\
      + '\n  %s -k exp=ascdaq123,run=12 -d epixm -o ./work # data on psana' % SCRNAME\
      + '\n  %s -k "{\'exp\':\'ascdaq123\', \'run\':12, \'detectors\':[\'epixm\',\'step_docstring\']}" -d epixm' % SCRNAME\
      + '\n  datinfo -k exp=ascdaq123,run=12 -d epixm'\
...
def argument_parser():
    ...
    d_nrecs    = 10000  # number of records in the data block
    d_dirrepo  = './work' #DIR_REPO_EPIXM320 # './work'
    d_logmode  = 'INFO'
    d_stepnum  = None
    d_stepmax  = 100
    d_stepskip = 0
    ...
    h_events   = 'total number of events in each step, default = %s' % str(d_events)
    h_evskip   = 'number of events to skip in the beginning of each step, default = %s' % str(d_evskip)
    h_evstep   = 'number of events per step, default = %s' % str(d_evstep)
    ...
    h_plotim   = 'plot image/s of none/raw/block/graphcs, 0/1/2/3, default = %d' % d_plotim
    h_figpref  = 'figurees filename prefix, default = %s' % str(d_figpref)
    ...
    parser.add_argument('-F', '--figpref',  default=d_figpref,  type=str,   help=h_figpref)
