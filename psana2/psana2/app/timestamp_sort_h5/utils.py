import os
from psana2 import utils

logger = utils.get_logger(name="timestamp_sort_h5.utils")


def get_dask_client(
    n_procs, partition="milano", n_jobs=1, memory="512GB", local_directory=None
):
...
def create_virtual_dataset(in_h5fname, out_h5fname, n_files):
    ...
    part_fnames = [
        os.path.join(out_dir, f"{out_basename}_part{i}.h5") for i in range(n_files)
    ]
        ...
        for array in in_t.list_nodes(group):
            ...
            if isinstance(array, (tb.array.Array,)):
                if len(in_f[entry_key].shape) > 0:
                    if in_f[entry_key].shape[0] == ts_len:
                        layouts[entry_key] = h5py.VirtualLayout(
                            shape=in_f[entry_key].shape, dtype=in_f[entry_key].dtype
                        )
            ...
            if flag_unaligned:
                ...
                if isinstance(array, (tb.array.Array,)):
                    ...
                    logger.debug(
                        f"Warning: found {entry_key} with unsupported data type {type(array)}. This dataset will not be included in the new sorted h5 output file."
                    )
                if val is not None:
                    logger.debug(
                        f"Create {entry_key} dataset for unaligned/scalar data type"
                    )
                    ...
                    if in_f[entry_key].shape[0] == ts_len:
                        ...
                        for i, filename in enumerate(part_fnames):
                            vsource = h5py.VirtualSource(
                                filename,
                                entry_key,
                                shape=part_files[i][entry_key].shape,
                            )
                            en = st + part_files[i][entry_key].shape[0]
                            logger.debug(
                                f"  part{i} shape:{part_files[i][entry_key].shape} {st=} {en=}"
                            )
                            layouts[entry_key][st:en] = vsource
                            st = en
                        out_f.create_virtual_dataset(
                            entry_key,
                            layouts[entry_key],
                        )
