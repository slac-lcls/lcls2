...
class CMWMainTabs(QWidget):
    ...
    def gui_selector(self, tab_name):
        ...
        if tab_name == 'CDB':
            from psana2.graphqt.CMWDBMain import CMWDBMain
            self.gui_win = CMWDBMain()
            w_height = 500

        elif tab_name == 'Configuration':
            from psana2.graphqt.CMWConfig import CMWConfig
            self.gui_win = CMWConfig()
            w_height = 500

        elif tab_name == 't-converter':
            from psana2.graphqt.QWDateTimeSec import QWDateTimeSec
            ...
            from psana2.graphqt.FMW1Main import FMW1Main
            self.gui_win = FMW1Main()
            #from psana2.graphqt.FMWTabs import FMWTabs
            #self.gui_win = FMWTabs()

        elif tab_name == 'Image':
            from psana2.graphqt.IVMain import IVMain
            #if cp.cmwmain is not None: cp.cmwmain.wlog.setVisible(False)
            self.gui_win = IVMain()

        elif tab_name == 'Text':
            from psana2.graphqt.QWTextBrowser import QWTextBrowser
            self.gui_win = QWTextBrowser()

        elif tab_name == 'XTC':
            from psana2.graphqt.DMQWMain import DMQWMain
    ...
    def view_data(self, data=None, fname=None):
        from psana2.pyalgos.generic.NDArrUtils import info_ndarr, np
        ...
        if data is None and fname is None:
            ...
            from psana2.pscalib.calib.MDBConvertUtils import info_dict
