...
class Test:
    def test_mpi(self, tmp_path):
        ...
        loop_based = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "user_loops.py"
        )
        ...
        run_early_termination = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "run_early_termination.py"
        )
        subprocess.check_call(
            ["mpirun", "-n", "3", "python", run_early_termination], env=env
        )
        ...
        subprocess.check_call(
            ["mpirun", "-n", "5", "python", loop_exhaustive_based], env=env
        )

        run_mixed_rate = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "run_mixed_rate.py"
        )
        subprocess.check_call(["mpirun", "-n", "5", "python", run_mixed_rate], env=env)

        run_chunking = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "run_chunking.py"
        )
        subprocess.check_call(["mpirun", "-n", "5", "python", run_chunking], env=env)

        run_intg_det = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "run_intg_det.py"
        )
