...
def info_ndarr(nda, name="", first=0, last=5, vfmt=None, spa=" ", sfmt=None):  #' mean:%.3f median:%.3f min:%.3f max:%.3f'):
    ...
    if nda is None:
        ...
        a = "" if last == 0 else str_formatted(nda, first=first, last=last, vfmt=vfmt, spa=spa)
        ...
        s = "%sshape:%s size:%d%s dtype:%s%s%s" % (_name, str(nda.shape), nda.size, sstat, nda.dtype, gap, a)
    return s
...
def shape_as_2d(sh):
    ...
    ndim = len(sh)
    if ndim > 2:
        return (int(size_from_shape(sh) / sh[-1]), sh[-1])
    if ndim < 2:
        return (1, sh[0])
    return sh
...
def shape_as_3d(sh):
    ...
    ndim = len(sh)
    if ndim > 3:
        return (int(size_from_shape(sh) / sh[-1] / sh[-2]), sh[-2], sh[-1])
    if ndim == 2:
        return (1, sh[0], sh[1])
    if ndim == 1:
        return (1, 1, sh[0])
...
def arr_rot_n90(arr, rot_ang_n90=0):
    ...
    elif rot_ang_n90 == 180:
        return np.flipud(np.fliplr(arr))
    elif rot_ang_n90 == 270:
...
def merge_masks(mask1=None, mask2=None, dtype=np.uint8):
    ...
    return mask if dtype == np.bool_ else np.asarray(mask, dtype)
...
def mask_neighbors_in_radius(mask, rad=5, ptrn="r"):
    ...
    for dr in range(-rad, rad + 1):
        ...
        for dc in range(-rad, rad + 1):
            if ptrn == "r" and (abs(dr) + abs(dc) > rad):
                continue
...
def mask_from_windows(ashape=(32, 185, 388), winds=None):
    ...
    if ndim < 2:
        print("ERROR in mask_from_windows(...):", " Wrong number of dimensions %d in the shape=%s parameter. Allowed ndim>1." % (ndim, str(shape)))
        return None
    ...
    if ndim == 2:
        for seg, rmin, rmax, cmin, cmax in winds:
            mask[rmin:rmax, cmin:cmax] = seg1[rmin:rmax, cmin:cmax]
        return mask

    elif ndim == 3:
        for seg, rmin, rmax, cmin, cmax in winds:
            mask[seg, rmin:rmax, cmin:cmax] = seg1[rmin:rmax, cmin:cmax]
        return mask
...
def list_of_windarr(nda, winds=None):
    ...
    if ndim == 2:
        return [nda] if winds is None else [nda[rmin:rmax, cmin:cmax] for (s, rmin, rmax, cmin, cmax) in winds]

    elif ndim == 3:
        return [nda[s, :, :] for s in range(ndim.shape[0])] if winds is None else [nda[s, rmin:rmax, cmin:cmax] for (s, rmin, rmax, cmin, cmax) in winds]
...
def subtract_bkgd(data, bkgd, mask=None, winds=None, pbits=0):
    ...
    if pbits:
        print("subtract_bkgd, fraction = %10.6f" % frac)
