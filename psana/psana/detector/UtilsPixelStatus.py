...
""" UtilsPixelStatus.py - utilities for command det_pixel_status
    ...
    import psana.detector.UtilsPixelStatus as us
...
from psana.detector.Utils import str_tstamp #, time, get_login, info_dict
...
#        return np.mean(block_good, axis=0, dtype=np.float32)
#
#    def feature_03(self):
#        logger.info("""Feature 3: dark RMS in good range""")
#        block_good = self.block[self.bool_good_frames,:] & self.databits
#        logger.info(info_ndarr(block_good, 'block of good records:', last=5))
#        return np.std(block_good, axis=0, dtype=np.float32)
#
#    def feature_04(self):
...
#
#        residuals = np.zeros_like(frame, dtype=np.float32)
#        wr, wc = self.shwind
...
#        block_res = np.zeros(shape_res, dtype=np.float32)
#
...
def event_loop(parser):
    ...
    repoman = args.repoman = init_repoman_and_logger(parser=parser, **kwargs)

    logger.info('optional parameters:\n ' + '\n  '.join(['%s: %s' % (k.ljust(12),str(v)) for k,v in kwargs.items()]))

    str_dskwargs = args.dskwargs  # dsname 'exp=xpplw3319:run=293'
    detname  = args.detname   # source
    events   = args.events
    evskip   = args.evskip
    stepnum  = args.stepnum
    stepmax  = args.stepmax
    steps    = eval('(%s)' % args.steps) # converts str like '5,6,7' to the list of steps (5,6,7)
    #evcode   = args.evcode
    segind   = args.segind
    ...
    for irun,orun in enumerate(ds.runs()):
      ...
      logger.debug('  run tstamp: %s' % ts_run)
      logger.debug('  now tstamp: %s' % ts_now)
      ...
      if args.databits is None: args.databits = odet.raw._data_bit_mask
      databits = args.databits
      logger.info('    det.raw._data_bit_mask BIT_MASK: %d or %s or %s' % (databits, oct(databits), hex(databits)))

      #dcfg = odet.raw._config_object() #ue.config_object_det(odet)

      for istep, ostep in enumerate(orun.steps()):
        nstep_tot += 1
        logger.info('\n==== step in run / total: %d / %d' % (istep, nstep_tot))
        logger.debug('    step.evt._seconds: %d' % ostep.evt._seconds)
        ...
        if steps is not None:
            if not(nstep_tot in steps):
                logger.info('==== Step:%02d is skipped, --steps=%s' % (nstep_tot, str(steps)))
                continue
        ...
        if stepnum is not None:
            if   nstep_tot < stepnum:
                logger.info('==== Step:%02d is skipped, --stepnum=%d' % (nstep_tot, stepnum))
                continue
            elif nstep_tot > stepnum:
                logger.info('==== Step:%02d loop is terminated, --stepnum=%d' % (nstep_tot, stepnum))
                break
        ...
        for ievt,evt in enumerate(ostep.events()):
          ...
          a = raw[:,145:,:192] & databits  # min:4811 max:10054
