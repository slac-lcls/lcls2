...
_fname_prefix = ucr.fname_prefix # calib_file_name
...
def fname_total(prefix, ct, gmode):
    return '%s-%s-%s.dat' % (prefix, ct, gmode)
...
def path_prefixes(fname_prefix, dir_offset, dir_peds, dir_plots, dir_gain, dir_rms, dir_status, dir_min, dir_max):
    return ['%s/%s' % (d, fname_prefix) for d in (dir_offset, dir_peds, dir_plots, dir_gain, dir_rms, dir_status, dir_min, dir_max)]
    ...
    for orun in ds.runs():
      ...
      for nstep_run, step in enumerate(orun.steps()): #(loop through calyb cycles, using only the first):
        ...
        for idx, panel_id in zip(segment_inds,segment_ids):
            ...
            fname_prefix = _fname_prefix(shortname, idx, tstamp_run, orun.expt, orun.runnum, dirname=None)
              # <dirname>/jungfrauemu_000001-s00-20250203095124-mfxdaq23-r0007
            logger.debug('\n  fname_prefix:%s' % fname_prefix)

            prefix_offset, prefix_peds, prefix_plots, prefix_gain, prefix_rms, prefix_status, prefix_min, prefix_max =\
                path_prefixes(fname_prefix, dir_offset, dir_peds, dir_plots, dir_gain, dir_rms, dir_status, dir_min, dir_max)
            ...
            if mode=='AHL_H': # evaluate AHL_L from AHL_H
                ...
                if all([v is not None for v in (offset_hl_h, offset_hl_l, gain_hl_h, gain_hl_l)]):
                    ...
                    fname = fname_total(prefix_peds, 'pedestals', 'AHL_L')
                ...
                if all([v is not None for v in (offset_ml_m, offset_ml_l, gain_ml_m, gain_ml_l)]):
                    ...
                    fname = fname_total(prefix_peds, 'pedestals', 'AML_L')
...
def deploy_constants(parser):
    ...
    for ind, panel_id in zip(panel_inds,panel_ids):
        ...
        fname_prefix = _fname_prefix(shortname, ind, _tstamp, exp, irun, dirname=None) # <dirname>/jungfrauemu_000001-s00-20250203095124-mfxdaq23-r0007
        prefix_offset, prefix_peds, prefix_plots, prefix_gain, prefix_rms, prefix_status, prefix_min, prefix_max =\
            path_prefixes(fname_prefix, dir_offset, dir_peds, dir_plots, dir_gain, dir_rms, dir_status, dir_min, dir_max)
        ...
        if 'c' in proc:
             add_links_for_gainci_fixed_modes(dir_gain, fname_prefix) # FH->AHL_H, FM->AML_M, FL->AML_L/AHL_L
