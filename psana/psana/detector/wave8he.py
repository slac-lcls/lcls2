...
class wave8he_raw_0_0_1(DetectorImpl):
    def __init__(self, *args):
        super(wave8he_raw_0_0_1, self).__init__(*args)
    ...
    def raw_all(self, evt) -> Array2d:
        ...
        for i in range(8):
            ...
            if func:
                wf = func(evt)
                if wf is None:
                    continue  # missing waveform
                if len(waveforms) > 0:
                    if wf.shape != waveforms[-1].shape:
                        return None  # waveforms not the same shape
                waveforms.append(wf)
        if len(waveforms) == 0:
            return None  # no waveforms present
        return np.stack(waveforms)


class wave8he_fex_0_0_1(DetectorImpl):
    """Wave8HE FEX data access with HLS processing firmware.

    Provides access to peak-based and integral-based feature extraction
    from the LCLS-HE HLS processor firmware.

    Individual Fields (36 total):
        Peak-based (26 fields):
            - peakPos_0..7: uint8 - Peak position (time sample index 0-254)
            - peakVal_0..7: float - Peak amplitude (firmware-scaled)
            - base_0..7: float - Baseline values (firmware-scaled)
            - posPeakX, posPeakY: float - Beam position from peaks (may be NaN)

        Integral-based (10 fields):
            - integral_0..7: float - Baseline-subtracted integrals (can be negative)
            - posIntegralX, posIntegralY: float - Beam position from integrals (may be NaN)

    Convenience Methods:
        - peakPos_all(evt): Get all 8 peak positions as array
        - peakVal_all(evt): Get all 8 peak values as array
        - base_all(evt): Get all 8 baseline values as array
        - integral_all(evt): Get all 8 integral values as array

    Example:
        >>> det = ds.Detector('wave8he')
        >>> for evt in run.events():
        ...     # Get arrays
        ...     peak_pos = det.wave8_fex.peakPos_all(evt)  # shape (8,)
        ...     peak_val = det.wave8_fex.peakVal_all(evt)  # shape (8,)
        ...     baseline = det.wave8_fex.base_all(evt)     # shape (8,)
        ...     integral = det.wave8_fex.integral_all(evt) # shape (8,)
        ...
        ...     # Get individual position fields
        ...     posX = det.wave8_fex.posPeakX(evt)  # may be NaN
        ...     posY = det.wave8_fex.posPeakY(evt)  # may be NaN
    """

    def __init__(self, *args):
        super(wave8he_fex_0_0_1, self).__init__(*args)
        self._add_fields()

    def _info(self, evt):
        """Return segment info for diagnostics."""
        segments = self._segments(evt)
        if segments is None:
            return None
        return segments[0]

    def peakPos_all(self, evt) -> Array1d:
        """Get all 8 peak positions as array.

        Args:
            evt: Event object

        Returns:
            Array1d: Peak positions (time sample indices 0-254), shape (8,), dtype uint8
            None: If no data available
        """
        vals = []
        for i in range(8):
            func = getattr(self, "peakPos_%d" % i, None)
            if func:
                val = func(evt)
                if val is not None:
                    vals.append(val)
    ...
    def peakVal_all(self, evt) -> Array1d:
        """Get all 8 peak values as array.

        Args:
            evt: Event object

        Returns:
            Array1d: Peak amplitudes (firmware-scaled), shape (8,)
            None: If no data available

        Note:
            Values already have firmware scaling (÷4) applied, representing physical values.
        """
        vals = []
        for i in range(8):
            func = getattr(self, "peakVal_%d" % i, None)
            if func:
                val = func(evt)
                if val is not None:
                    vals.append(val)
    ...
    def base_all(self, evt) -> Array1d:
        """Get all 8 baseline values as array.

        Args:
            evt: Event object

        Returns:
            Array1d: Baseline values (firmware-scaled), shape (8,)
            None: If no data available
    ...
    def integral_all(self, evt) -> Array1d:
        """Get all 8 integral values as array.

        Args:
            evt: Event object

        Returns:
            Array1d: Baseline-subtracted integrals (can be negative), shape (8,)
            None: If no data available

        Note:
            Values are baseline-subtracted and can be negative.
            Values already have firmware scaling (÷4) applied, representing physical values.
        """
        vals = []
        for i in range(8):
    ...
    def __init__(self, *args):
        ...
        for i in range(16):
            ...
            if self._chan_enable & (1 << i):
                ...
                channel_delay_desc: Any = getattr(self._seg_configs()[0].config.read_only, f"Delay{i}")
                n_samples_desc: Any = getattr(self._seg_configs()[0].config.read_only, f"NumberOfSamples{i}")
