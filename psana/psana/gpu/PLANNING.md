...
### `PS_EB_PER_NODE`: multiple EBs per node — the knee saturates near 3
...
optimum `k` is a **crossover that rises with node count at first, then saturates
near 3**:
...
windows (802.5 vs 791.5, ~1.4%), so the peak is real, not window drift. 4-node:
...
EB parallelism past ~3/node stops paying — the marginal EB's lost BD slot +
