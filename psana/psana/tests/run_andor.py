...
def my_smalldata(data_dict):
    ...
    if "sum_atmopal" in data_dict:
        ...
        myplot = XYPlot(
            0,
            f"Atmopal (sum) run:{runnum}",
            range(len(atmopal_sum)),
            atmopal_sum,
        )
    ...
    for nstep, step in enumerate(myrun.steps()):
        ...
        for nevt, evt in enumerate(step.events()):
            ...
            if andor_img is not None:
                ...
                print(
                    f"BD{rank-1}: #andor: {cn_andor_events} #atm: {cn_atm_events} #intg:{cn_intg_events} st: {ts_st} en:{evt.timestamp}"
                )
