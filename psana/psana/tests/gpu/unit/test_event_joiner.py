...
class TestEventPoolLeases:
    ...
    def test_retire_next_waits_for_d2h_before_recycle(self, monkeypatch):
        ...
        pool.retire_next()
        assert pending_d2h._synced, \
            "retire_next() must synchronise D→H before recycling the slot"
...
class TestD2hPipeline:
    ...
    def test_pipeline_sets_pending_d2h(self):
        ...
        for ctx in [ctx0, ctx1]:
            ...
            assert result._pending_d2h is not None, \
                "_pending_d2h must be set so on_cpu can sync lazily"
    ...
    def test_calib_done_waited_before_d2h(self):
        ...
        assert calib in pipe._d2h_stream.wait_events, \
            "D→H stream must call wait_event(calib_done) before memcpyAsync"
