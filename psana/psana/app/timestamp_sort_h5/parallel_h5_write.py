            ...
            if isinstance(array, (tb.array.Array, )):
                ...
                if len(in_f[ds_path].shape) > 0:
                    if in_f[ds_path].shape[0] == ts_len:
                        ...
                        t3 = time.time()
                        logger.debug(f'key:{ds_path} dask slicing:{t2-t1:.2f}s. writing:{t3-t2:.2f}s.')
                else:
...
en = time.time()
logger.debug(f"DONE setup_cluster:{t0-st:.2f}s. slice_and_write: {en-t0:.2f}s. total: {en-st:.2f}s.")
common_comm.Barrier()
common_comm.Abort(1)
