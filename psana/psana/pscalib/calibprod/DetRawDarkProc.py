...
class DetRawDarkProc :
    ...
    def __init__(self, orun, detname, args):
                 #ofname, events, evskip, intlow, inthig, rmslow, rmshig, fraclm, nsigma,
    ...
    def print_attrs(self) :
        msg = '\n%s\nAttributes of the %s object' % (60*'_', self.__class__.__name__)\
            ...
            + '\ncontrol bit-word for verbosity  : %d'   % self.verbos\
    ...
    def summary(self, evt) :
        ...
        if self.verbos & 1 : logger.info('Data summary for %s is completed, dt=%7.3f sec' % (self.detname, time()-t0_sec))
