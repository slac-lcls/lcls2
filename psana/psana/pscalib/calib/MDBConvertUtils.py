...
def numpy_scalar_types():
    """Returns a tuple of numpy scalar types covering int, uint, float, and complex categories.
    Replaces np.sctypes which was removed in NumPy 2.0.
    Extended precision types (np.longdouble, np.clongdouble) are included when available
    and distinct from the standard double-precision types (formerly np.float128/np.complex256).
    """
    types = [
        np.int8,
        np.int16,
    ...
    for t in (np.longdouble, np.clongdouble):
        if t not in types:
            types.append(t)
    return tuple(types)
...
def compare_dicts(d1, d2, gap="  "):
    ...
    for k1, v1 in d1.items():
        ...
        if isinstance(v1, dict):
            ...
            print("%s are equal: %s" % (s_key, np.array_equal(v1, v2)))
        else:
            print("%s are equal: %s" % (s_key, v1 == v2))
...
def deserialize_value(d):
    ...
    return (
        int(data)
        if dtype in TYPES_OF_INT
        else float(data)
        if dtype in TYPES_OF_FLOAT
        else str(data)
        if dtype == "str"
        else bool(data)
        if dtype == "bool"
...
def serialize_dict(d):
    ...
    for k, v in d.items():
        ...
        if isinstance(v, str):
            ...
            d[k] = serialize_numpy_array(v)
        elif isinstance(v, bytes):
            d[k] = v
        elif isinstance(v, (int, float, str, bool)):
            d[k] = serialize_value(v)
        elif isinstance(v, NUMPY_SCALAR_TYPES):
            d[k] = serialize_numpy_value(v)
...
def info_dict(d, offset="  ", s=""):
    ...
    for k, v in d.items():
        if isinstance(v, dict):
            s = info_dict(v, offset=offset + "  ", s=s)
        else:
...
if __name__ == "__main__":
    ...
    def test_serialize_dict():
        d = {"val": 123, "nda1": aranged_array(shape=(3, 4), dtype=np.int32), "nda2": random_standard(shape=(2, 3), mu=100, sigma=10, dtype=np.float32)}
        print("initial dict:")
        print_dict(d)
        serialize_dict(d)
        print(80 * "_")
    ...
    def usage():
        return (
            "Use command: python %s <test-number>, where <test-number> = 0,1,2,3" % sys.argv[0]
...
if __name__ == "__main__":
    ...
    logger.info("%s\nTest %s:" % (50 * "_", tname))
    if tname == "0":
        test_serialize_numpy_value()
    elif tname == "1":
        test_serialize_numpy_array()
    elif tname == "2":
        test_serialize_dict()
    elif tname == "3":
        test_serialize_value()
    else:
        logger.info("Not-recognized test name: %s" % tname)
