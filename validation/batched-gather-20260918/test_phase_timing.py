...
@pytest.mark.parametrize('variant,kind', [('O', 'detector'), ('O', 'parse'), ('G', 'detector'), ('G', 'parse')])
def test_historical_transform_preserves_statements(variant, kind):
    root = (Path(__file__).parent / 'install_baseline/lib/python3.9/site-packages/psana/gpu' if variant == 'O' else Path(__file__).resolve().parents[2] / 'psana/psana/gpu')
