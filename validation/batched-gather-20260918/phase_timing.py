...
class Ranges(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        ...
        if self.kind == 'detector' and self.variant in ('B', 'O', 'G'):
    ...
    def visit_With(self, node):
        if self.kind == 'detector' and self.variant == 'G' and any(
...
def transform(source, kind, variant):
    ...
    if kind == 'detector' and variant in ('B', 'O', 'G'):
        expected.update({'detector.zero_present': 1, 'detector.zero_missing': 1})
    if kind == 'detector' and variant == 'G':
