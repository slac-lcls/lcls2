"""Matched warm-only A/B+gather/zero-wrapper controls, CPU/NVTX timing, then separate Nsight runs."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--instrumented-repeats', type=int, default=0)
    ...
    sources = dict(A=HISTORICAL/'sources/a', G=REPO, Z=REPO)
    prefixes = {v: HERE/'installs'/v for v in 'AGZ'}
    results = []
    os.environ['BENCH_CPU_AFFINITY'] = ','.join(map(str, sorted(os.sched_getaffinity(0))))
    orders = ('AGZ', 'GZA', 'ZAG', 'ZGA', 'GAZ', 'AZG')
    ...
    provenance = dict(node=os.uname().nodename, constants=str(constants),
                      ...
                      revisions={v: subprocess.check_output(['git', '-C', str(sources[v]),
                                                            'rev-parse', 'HEAD'], text=True).strip() for v in 'AGZ'},
    ...
    provenance['builds'] = json.loads((HERE/'builds.json').read_text())
    ...
    execute('Z-locator-correctness', [sys.executable, '-m', 'pytest', '-q',
                str(REPO/'psana/psana/tests/gpu/integration/test_batched_locators.py'),
                str(REPO/'psana/psana/tests/gpu/integration/test_batched_gather.py')], env('Z'))
    stage = a.stage
    if stage is None:
    ...
    for mode in (('off', 'cpu-nvtx') if a.instrumented_repeats or not a.skip_trace else ('off',)):
        for variant in 'AGZ':
            case(variant, mode, 0, check=True)
    print('CORRECTNESS_PASS A/B+gather/zero-wrapper for requested timing modes', flush=True)
