"""Verify the completed one-allocation A/batched-gather/zero-wrapper comparison without speed thresholds."""
import argparse
import csv
import hashlib
import json
...
def audit(root):
    ...
    installed = {variant: {str(Path(path).relative_to(provenance['prefixes'][variant])): digest
                          for path, digest in hashes.items()}
                 for variant, hashes in provenance['installed_hashes'].items()}
    assert installed['G'].keys() == installed['Z'].keys()
    changed = [path for path in installed['G'] if installed['G'][path] != installed['Z'][path]]
    assert changed == ['lib/python3.9/site-packages/psana/gpu/gpudgram/parser.py']
    assert installed['Z'][changed[0]] == provenance['builds']['Z']['parser_sha256']
    assert '33 passed' in (root / 'Z-locator-correctness.log').read_text()
    ...
    assert len(traces) == (0 if args['skip_trace'] else 3)
    ...
    for variant in 'AGZ':
        for mode in (('off', 'cpu-nvtx') if args['instrumented_repeats'] or not args['skip_trace'] else ('off',)):
    ...
    return dict(gpu_uuid=next(iter(gpu_uuids)), samples=len(results), correctness_preflights=3 * (2 if args['instrumented_repeats'] or not args['skip_trace'] else 1), node=provenance['node'],
...
if __name__ == '__main__':
    ...
    print(f"AUDIT_PASS: {result['samples']} samples, {result['correctness_preflights']} preflights, matched workload/placement, unchanged installs")
