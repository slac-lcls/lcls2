...
def main():
    ...
    sources = dict(A=HISTORICAL/'sources/a', B=HISTORICAL/'sources/b', O=REPO)
    prefixes = dict(A=sources['A']/'install_acceptance', B=sources['B']/'install_psana', O=REPO/'install_psana')
    results = []
    os.environ['BENCH_CPU_AFFINITY'] = ','.join(map(str, sorted(os.sched_getaffinity(0))))
    orders = ('ABO', 'BOA', 'OAB', 'OBA', 'BAO', 'AOB')
    ...
    provenance = dict(node=os.uname().nodename, constants=str(constants),
                      ...
                      revisions={v: subprocess.check_output(['git', '-C', str(sources[v]),
                                                            'rev-parse', 'HEAD'], text=True).strip() for v in 'ABO'},
    ...
    print('CORRECTNESS_PASS A/B/B+optimization with and without timing hooks', flush=True)
