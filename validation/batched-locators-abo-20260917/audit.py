"""Verify the completed one-allocation A/B/B+ comparison without speed thresholds."""
import argparse
import csv
import hashlib
import json
...
def audit(root):
    ...
    for repetition in range(1, args['repeats'] + 1):
        ...
        for mode in (modes if repetition % 2 else tuple(reversed(modes))):
            for variant in provenance['orders'][(repetition - 1) % 6]:
                expected.append(f'{variant}-warm-{mode}-r{repetition}')
    assert [r['case'] for r in results] == expected
    assert args['skip_trace'] and all(not r['nsight_capture'] for r in results)
    ...
    return dict(gpu_uuid=next(iter(gpu_uuids)), samples=len(results), correctness_preflights=6, node=provenance['node'],
...
if __name__ == '__main__':
    ...
    print(f"AUDIT_PASS: {result['samples']} samples, six preflights, matched workload/placement, unchanged installs")
