"""Matched warm-only B/B+optimization controls, CPU/NVTX timing, then separate Nsight runs."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    a = p.parse_args()
    if a.events < 100 or a.repeats < 1:
    ...
    sources = dict(B=HISTORICAL/'sources/b', O=REPO)
    prefixes = dict(B=sources['B']/'install_psana', O=REPO/'install_psana')
    results = []
    ...
    provenance = dict(node=os.uname().nodename, constants=str(constants),
                      ...
                      revisions={v: subprocess.check_output(['git', '-C', str(sources[v]),
                                                            'rev-parse', 'HEAD'], text=True).strip() for v in 'BO'},
    ...
    print('CORRECTNESS_PASS B/B+optimization with and without timing hooks', flush=True)
    for repetition in range(1, a.repeats + 1):
        for mode in (('off', 'cpu-nvtx') if repetition % 2 else ('cpu-nvtx', 'off')):
            for variant in ('BO' if repetition % 2 else 'OB'):
