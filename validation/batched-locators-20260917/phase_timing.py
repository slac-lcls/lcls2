...
class Ranges(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        ...
        if self.kind == 'detector' and self.variant in ('B', 'O'):
...
def transform(source, kind, variant):
    ...
    if kind == 'detector' and variant in ('B', 'O'):
...
def install(variant, nvtx=None):
    if variant not in ('B', 'O'):
        raise ValueError('timing hooks support only frozen A/B revisions')
