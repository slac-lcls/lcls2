...
@pytest.mark.parametrize('variant,kind', [('B', 'detector'), ('B', 'parse'), ('O', 'detector'), ('O', 'parse')])
def test_historical_transform_preserves_statements(variant, kind):
    root = (Path('/sdf/home/m/monarin/lcls2_worktree/psana2-gpu-d2h-pipeline/validation/perf-acceptance-20260916/sources/b') if variant == 'B' else Path(__file__).resolve().parents[2]) / 'psana/psana/gpu'
