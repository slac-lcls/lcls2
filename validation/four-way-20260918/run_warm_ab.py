"""Matched warm-only A/B/B+/B+gather controls, CPU/NVTX timing, then separate Nsight runs."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    sources = dict(A=HISTORICAL/'sources/a', B=HISTORICAL/'sources/b', O=REPO, G=REPO)
    prefixes = {v: HERE/'installs'/v for v in 'ABOG'}
    results = []
    os.environ['BENCH_CPU_AFFINITY'] = ','.join(map(str, sorted(os.sched_getaffinity(0))))
    orders = ('ABGO', 'BOAG', 'OGBA', 'GAOB')
    ...
    provenance = dict(node=os.uname().nodename, constants=str(constants),
                      ...
                      revisions={v: subprocess.check_output(['git', '-C', str(sources[v]),
                                                            'rev-parse', 'HEAD'], text=True).strip() for v in 'ABOG'},
    ...
    print('CORRECTNESS_PASS A/B/B+/B+gather for requested timing modes', flush=True)
