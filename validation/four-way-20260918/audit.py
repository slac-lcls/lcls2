"""Verify the completed one-allocation A/B/B+/batched-gather comparison without speed thresholds."""
import argparse
import csv
import hashlib
import json
...
def audit(root):
    ...
    assert len(traces) == (0 if args['skip_trace'] else 4)
    ...
    return dict(gpu_uuid=next(iter(gpu_uuids)), samples=len(results), correctness_preflights=4 * (2 if args['instrumented_repeats'] or not args['skip_trace'] else 1), node=provenance['node'],
