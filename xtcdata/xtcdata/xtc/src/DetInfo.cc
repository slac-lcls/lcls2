...
bool DetInfo::operator==(const DetInfo& s) const
{
    return _phy == s._phy;
}

uint32_t DetInfo::processId() const
{
    return _log & 0xffffff;
}

DetInfo::Detector DetInfo::detector() const
{
    return (Detector)((_phy & 0xff000000) >> 24);
}
DetInfo::Device DetInfo::device() const
{
    return (Device)((_phy & 0xff00) >> 8);
}
uint32_t DetInfo::detId() const
{
    return (_phy & 0xff0000) >> 16;
}
uint32_t DetInfo::devId() const
{
    return _phy & 0xff;
}
...
{
    static const char* _detNames[] = { "NoDetector",
                                       "AmoIMS",
                                       "AmoGD",
                                       "AmoETOF",
                                       "AmoITOF",
...
const char* DetInfo::name(Device dev)
{
    static const char* _devNames[] = {
        "NoDevice", "Evr",      "Acqiris",  "Opal1000",    "Tm6740",   "pnCCD",    "Princeton",
        "Fccd",     "Ipimb",    "Encoder",  "Cspad",       "AcqTDC",   "Xamps",    "Cspad2x2",
        "Fexamp",   "Gsc16ai",  "Phasics",  "Timepix",     "Opal2000", "Opal4000", "OceanOptics",
        "Opal1600", "Opal8000", "Fli",      "Quartz4A150", "Andor",    "USDUSB",   "OrcaFl40",
        "Imp",      "Epix",     "Rayonix",  "EpixSampler", "Pimax",    "Fccd960",  "Epix10k",
        "Epix100a", "EpixS",    "Gotthard", "DualAndor",   "Wave8",    "LeCroy",   "ControlsCamera",
...
const char* DetInfo::name(const DetInfo& src)
{
    ...
    snprintf(_name, MaxLength, "%s-%u|%s-%u", name(src.detector()), (unsigned)src.detId(),
